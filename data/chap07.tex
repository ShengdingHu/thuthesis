...
\begin{figure}[!htpb]
    \centering
    \includegraphics[width=0.8\linewidth]{fpqafigs/intrograph_v4.pdf}
...
\begin{table}[]
    ...
    \caption{不同模型在错误前提问题上的表现对比。\textsc{Macaw}-FPQ 是经过微调的模型，能够正确识别并反驳错误前提问题。}
    \label{tab:model_comparison}
\end{table}


\subsection{预训练语言模型对错误前提问题的原始响应}
\label{sec:pilot}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{fpqafigs/annotate.pdf}
...
\begin{table}[]
    \centering
    \caption{错误前提问题的分类及示例。本表省略了“其他”类别。}
...
\subsection{数据集统计}
最终的数据集名为 FalseQA，包含 2365 个问题对。表~\ref{tab:FalseQA_examples} 展示了错误前提问题数据集的一个示例。本文随机将数据集按 5:2:3 的比例划分为训练集、验证集和测试集。统计摘要如表~\ref{tab:statisticsofFPQ} 所示。

\definecolor{greycolor}{rgb}{0.917, 0.917, 0.917}
% \newcolumntype{g}{>{\columncolor{greycolor}}c}
\begin{table}[!htbp]
    \caption{FalseQA 数据集的统计信息。词数通过 NLTK~\cite{bird-loper-2004-nltk} 计算。}
    \centering
...
\begin{table*}[!htbp]
    \centering
    \caption{示例问题对（错误前提问题与真实问题）及其源词、解释/答案。}
    \resizebox{\textwidth}{!}{
...
\begin{figure}
    \centering
    \includegraphics[width=0.69\linewidth]{fpqafigs/exp2.pdf}
...
& {\quad{+Binary Loss}} & \std{59.0}{5.3} & \myemph{\std{56.3}{1.2}} & \std{27.0}{1.6}\\
& \textsc{Macaw}-3B  & \myemph{\std{41.9}{22.3}} & \std{56.8}{3.4} & \std{29.1}{3.0}  \\
& {\quad{+Binary Loss}} & \std{40.5}{21.8} & \myemph{\std{61.5}{7.7}} & \myemph{\std{32.0}{1.3}}\\
& \textsc{Macaw}-11B & \myemph{\std{64.5}{36.9}} &  \std{59.2}{9.0} & \myemph{\std{36.2}{5.2}} \\
& {\quad{+Binary Loss}} & \std{49.0}{19.6} & \myemph{\std{64.1}{7.2}} & \std{33.8}{0.5}\\
\midrule
\multirow{6}{*}{256} 
& OPT-2.7B & \std{56.8}{5.3} & \std{56.9}{2.0} & \std{29.5}{0.4} \\
& {\quad{+Binary Loss}} & \myemph{\std{62.5}{5.5}} & \myemph{\std{67.8}{1.6}} & \myemph{\std{29.7}{0.5}}  \\
&\textsc{Macaw}-3B  &  \std{69.5}{7.5}&\std{73.5}{1.7}&\std{34.5}{1.3}  \\
& {\quad{+Binary Loss}} &  \myemph{\std{72.6}{8.7}} & \myemph{\std{76.5}{2.3}} & \myemph{\std{35.3}{1.5}} \\
& \textsc{Macaw}-11B  & \std{77.3}{13.0} & \std{76.2}{1.9} & \std{35.0}{2.0}  \\
& {\quad{+Binary Loss}} & \myemph{\std{81.3}{4.6}} & \myemph{\std{79.2}{0.2}} & \myemph{\std{38.4}{0.7}} \\
\midrule
\multirow{6}{*}{1187} 
& OPT-2.7B & \myemph{\std{76.2}{4.1}} & \std{70.8}{0.9} & \myemph{\std{34.2}{0.6}} \\
& {\quad{+Binary Loss}} & \std{75.9}{4.9} & \myemph{\std{75.3}{0.5}} & \std{34.0}{1.1} \\
& \textsc{Macaw}-3B  & \myemph{\std{81.8}{7.3}} & \std{80.6}{1.2} & \myemph{\std{39.2}{1.9}}  \\
& {\quad{+Binary Loss}} & \std{80.9}{1.2} & \myemph{\std{84.2}{0.7}} & \std{38.1}{1.0} \\
& \textsc{Macaw}-11B  & \myemph{\std{90.7}{5.2}} &  \std{83.6}{0.8} & \std{41.9}{0.6}  \\
& {\quad{+Binary Loss}} & \std{88.8}{1.8} & \myemph{\std{87.1}{0.9}} & \myemph{\std{42.0}{0.7}} \\
\bottomrule
    \end{tabular}
}
    \caption{联合错误前提问题识别与解释生成。更好的结果以 \colorbox{emphcolor}{绿色} 标出。}
    \label{tab:model_stimulation}
\end{table} 


\subsection{通过解释回答错误前提问题}
\label{exp:answerfpq}
...
\begin{table*}[]
    \centering
    \caption{测试集中随机抽样的错误前提问题。“参考解释”列展示了两个参考解释之一。“分类结果”列表示模型是否成功将问题分类为错误前提问题。“生成解释”列提供了模型对反驳情况的生成解释以及对非反驳情况的生成答案。}
    \resizebox{\textwidth}{!}{
...
\begin{table}[htbp]
    \centering
    \caption{在不同示例数量下的上下文学习性能。更好的结果以 \colorbox{emphcolor}{绿色} 标出。}
\resizebox{0.65\linewidth}{!}{
...
\begin{figure*}[htbp]
    \centering
    \includegraphics[width=0.97\textwidth]{fpqafigs/category.pdf}
    \caption{预训练语言模型在不同错误类型（左）和问题形式（右）上的准确率得分。}
    \label{fig:category}
\end{figure*}

\subsection{上下文学习（In-context Learning）}
...
\begin{table*}[!htbp]
    \centering
    \caption{使用 FalseQA 数据和数据回放技术微调后的结果。更好的结果以 \colorbox{emphcolor}{绿色} 标出。}
